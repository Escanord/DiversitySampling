...
def plot_sample(num_samples, uniform, race, uniform_errors, race_errors):
    ...
    plt.plot(iters, uniform, label="Uniform sampling", color = 'blue')
    plt.errorbar(iters, uniform, yerr = uniform_errors, label="Uniform sampling", color = 'blue')
    plt.show(block=True)
    plt.plot(iters, race, label="Race Sampling", color = 'green')
...
def main():
    ...
    for i in range(1, 31):
        ...
        uniform_samples.append(round(uniform_ave / 10, 3))
        race_samples.append(round(race_ave / 10, 3))
    ...
    # plot_sample(20, uniform_samples, race_samples, uniform_errors, race_errors)
